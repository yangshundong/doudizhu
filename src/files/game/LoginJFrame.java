...
import files.util.CodeUtil;
import files.util.db.Db1;
...
public class LoginJFrame extends JFrame implements MouseListener {
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == login) {
            ...
            ResultSet set = db.select("select * from user", objs);
            try {
                while (set.next()) {
                    int t_id = set.getInt("id");
                    String name = set.getString("username");
                    String password = set.getString("password");
                    currentuser.add(new User(name, password));
                }
            ...
            if (currentuser.contains(userInfo)) {
                //关闭当前登录界面
